"""EffectSampler — per-feature contribution workhorse (issue 0005 / GitHub #6).

Draws T posterior weight samples and returns per-feature contribution tensors.
Pure function of (model, data, T): no mutation of model parameters or mode.
...
class EffectSampler:
    """Callable that draws T posterior samples and returns per-feature contributions.

    Args:
        None — instantiate once, call with different (model, data, T) triples.

    Example::

        sampler = EffectSampler()
        samples = sampler(model, data)           # T defaults to T_predict = 200
        samples = sampler(model, data, T=500)    # override
        # samples["x1"].shape == (T, n, param_count)
    """
