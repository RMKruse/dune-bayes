...
if __name__ == "__main__":
    ...
    # Custom for Dataset
    # Load data
    data = pd.read_csv("data/MELBOURNE_HOUSE_PRICES_LESS.csv")
    data.pop("Address")
    data.pop("Date")

    data["Price"] = np.log(data["Price"])
    scaler = StandardScaler().fit(np.array(data["Price"]).reshape(-1, 1))
    data["Price"] = scaler.transform(np.array(data["Price"]).reshape(-1, 1)).flatten()
    data.dropna(inplace=True)
    data = data.reset_index(drop=True)
    targets = np.array(data.pop("Price"))
